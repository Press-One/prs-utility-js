...
const { bufToHex, hexToBuf, dumpBuf, randomBytes, getSortedQS } = require('./util');
...
const signHash = (hash, privateKey) => {
  ...
  const sigHex = signature.r.toString(16, 32) + signature.s.toString(16, 32) + signature.recoveryParam.toString();
...
const signBlockData = (data, privateKey) => {
  // get hash
  const sqs = getSortedQS(data);
  const hash = keccak256(sqs);
